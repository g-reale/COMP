        ...
        if(holds_alternative<nonterminal_t>(node->data)){
            ...
            switch (nonterminal){
                ...
                case nonterminal_t::FUNCTION_ACTIVATION:{
                    ...
                    variable_t function;
                    find(name,function);
                    if(function.segment != (size_t)segment_t::VOID_FUNCTIONS && 
                       function.segment != (size_t)segment_t::INT_FUNCTIONS && 
                       function.segment != (size_t)segment_t::SCALARS) //allow scalars to be used as function pointers
    ...
    for(size_t i = 0; i < segments.size(); i++){
        ...
        switch ((segment_t)i){
            ...
            case segment_t::RESERVED:
                binary[(size_t)reserved_t::RELATIVE_START   + offset] = offsets[(size_t)segment_t::USER_POITERS];
                ...
                binary[(size_t)reserved_t::MEM              + offset] = 0;
                break;
...
std::ostream& operator<<(std::ostream& os, const Assembler& assembler){
    ...
    size_t relative = assembler.binary[2];

    os << "0: " << main << " (entrypoint)" << std::endl;
    os << "1: " << assembly << " (code start)" << std::endl;
    os << "2: " << relative << " (relative start)" << std::endl;

    for(size_t i = 3; i < assembly; i++)
        os << i << ": " << assembler.binary[i] << std::endl;

    for(size_t i = assembly; i < assembler.binary.size(); i++){
        size_t word = assembler.binary[i];
        os << i << ": " << std::setw(11) << std::setfill('0') << word << "\t" << (operator_t)((word >> 3 * Assembler::ARGUMENT_LENGTH) & 0x3FF) << "(" << ((word >> 2 * Assembler::ARGUMENT_LENGTH) & 0x3FF) << "," << ((word >> Assembler::ARGUMENT_LENGTH) & 0x3FF)  << "," << (word & 0x3FF) << ")" << endl;
    }
    return os;
}