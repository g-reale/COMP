...
void popContext(semantis * s, size_t line){
    ...
    if(!returned && s->context_stack[s->top]->explicit){
        fprintf(stderr,"ERRO SEMANTICO: função inteira \"%s\" encerra sem retorno LINHA: %zu\n",
...
variable * declareVar(semantis * s, size_t line){
    ...
    if(isUnique(*list,*size,s->lexeme) != NULL){
        fprintf(stderr,"ERRO SEMANTICO: simbolo \"%s\" já foi declarado nesse contexto LINHA: %zu\n",s->lexeme,line);
        s->success = 0;
        return NULL;
    }
    else
    ...
    if(s->category == OPEN_ROUND){
        ...
        if(s->main_declared){
            fprintf(stderr,"ERRO SEMANTICO: funcão \"%s\" declarada após a main LINHA: %zu\n",s->lexeme,line);
            s->success = 0;
        }

        if(index == main_hash && strcmp(s->lexeme,"main") == 0){
            if(s->type != VOID){
                fprintf(stderr,"ERRO SEMANTICO: função main deve retornar void LINHA: %zu\n",line);
                s->success = 0;
            }
            s->main_declared = 1;
        }
...
variable * searchVar(semantis * s, size_t line){
    ...
    if(var == NULL){
        fprintf(stderr,"ERRO SEMANTICO: simbolo \"%s\" utilizado mas nunca declarado LINHA: %zu\n",s->lexeme,line);
        s->success = 0;
        return NULL;
    }
